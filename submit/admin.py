...
class SubmissionFileLinkWidget(forms.Widget):
	...
	def render(self, name, value, attrs=None):
		try:
			sfile = self.original_value
			#sfile = SubmissionFile.objects.get(pk=value)
			text = u'<a href="%s">%s</a><br/><table border=1><tr>'%(sfile.get_absolute_url(), sfile.basename())
			text += u'<td><h3>Compilation test</h3><pre>%s</pre></td>'%(sfile.test_compile)
...
class SubmissionAdmin(admin.ModelAdmin):	
	list_display = [authors, course, 'assignment', 'state']
	list_filter = (SubmissionStateFilter,'assignment')
	filter_horizontal = ('authors',)
	readonly_fields = ('assignment','submitter','authors','notes')
	fields = ('assignment','submitter','authors','notes','file_upload','state','grading','grading_notes')
