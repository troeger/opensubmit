...
class AssignmentAdminForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            # makes only sense if this is not a new assignment to be created
            url = self.instance.validity_test_url_absolute()
            if url:
                self.fields['attachment_test_validity'].widget.template_with_initial = (
                    '%(initial_text)s: <a href="' + url + '">%(initial)s</a> '
            ...
            url = self.instance.full_test_url_absolute()
            if url:
            ...
            url = self.instance.url_absolute()
            if url:
...
def view_links(obj):
    ...
    result = format_html('')
    result += format_html('<a href="%s" style="white-space: nowrap">Show duplicates</a><br/>' %
                          reverse('duplicates', args=(obj.pk,)))
    result += format_html('<a href="%s" style="white-space: nowrap">Show submissions</a><br/>' %
                          obj.grading_url())
    result += format_html('<a href="%s" style="white-space: nowrap">Download submissions</a>' %
                          reverse('assarchive', args=(obj.pk,)))
    return result
...
class AssignmentAdmin(ModelAdmin):
    list_display = ['title', course, 'soft_deadline', 'hard_deadline',
                    'gradingScheme', num_authors, num_subm, num_finished, num_unfinished, view_links]
    ...
    def get_queryset(self, request):
        ...
        if not request.user.is_superuser:
            qs = qs.filter(course__active=True).filter(
                Q(course__tutors__pk=request.user.pk) | Q(course__owner=request.user)).distinct()
