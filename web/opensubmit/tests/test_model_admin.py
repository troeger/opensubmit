...
class ModelAdminTestCase(SubmitTutorTestCase):
    ...
    def testGradingBackend(self):
        ...
        self.assertEqual(grading_schemes(self.passGrade),
                         self.passFailGrading.title)
...
class SubmissionModelAdminTestCase(SubmitTutorTestCase):
    ...
    def setUp(self):
        ...
        self.sub1 = self.createSubmission(
            self.current_user, self.openAssignment)
        self.sub2 = self.createSubmission(
    ...
    def testSetFullPendingNoneMatching(self):
        # Only one of the submission assignments has validation configured
        self.submadm.setFullPendingStateAction(
            self.request, Submission.objects.filter(state=Submission.SUBMITTED))
    ...
    def testStateFilter(self):
        ...
        graded_count = SubmissionStateFilter(self.request, {'statefilter': 'graded'}, Submission, None).queryset(
    ...
    def testAssignmentFilter(self):
        ...
        submfilter = SubmissionAssignmentFilter(
            self.request, {'assignmentfilter': self.sub1.assignment.pk}, Submission, None)
        sublist = submfilter.queryset(
            self.request, Submission.objects.all()).values_list('pk', flat=True)
